...
function createAuction(){
	...
	model.set('_winner', (Math.random()*Math.pow(10,19)).toString(32));
}

var query = model.query('users').where('bid').gte(1).sort('bid', 'asc');
model.subscribe(query, function(err){
	var users=arguments;
	model.on('set', 'users.*.bid', function(){
		model.set('auction.0.winner', 'inside subscribe:'+Math.random().toString(32)+": "+users.length+users[0]);
		if(users.length == 2) model.set("auction.0.winner", users[1]["id"])
