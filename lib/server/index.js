...
function createAuction(){
	...
	model.ref('_winner', "users.default");
}

var query = model.query('users').where('bid').gte(1).sort('bid', 'asc');
model.subscribe('users', function(err){/*
	var users=arguments;
	model.on('set', 'users.*.bid', function(username, newbid){
		debugger;
		model.set('users.'+username+".lastupdated", new Date());
		if(users.length == 2){
			debugger;
			model.set("auction.0.winner", users[1]["id"]);

		}
		else{
			debugger;
			for(var i=1; i<=users.length-2; i++){
				if(users[i]["bid"] != users[i+1]["bid"]) model.set('auction.0.winner', users[i]["id"]);
				else model.set('auction.0.winner', "default");
			}
		}
	});*/

	model.on('set', 'users.*.bid', function(username, bid){
		var players = users.slice().sort(function(a,b){
			return a.bid - b.bid;
		});

		for(var i=0; i<players.length-2; i++){
			if(players[i]['bid'] !== players[i+1]['bid']) return players[i]['bid'];
		}
		return "default";console.log("USERS:"+users);
	});
});